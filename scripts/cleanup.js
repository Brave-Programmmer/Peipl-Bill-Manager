...
componentFiles.forEach((file) => {
  ...
  Object.keys(functions).forEach((funcName) => {
    if (functions[funcName] > 1) {
      issues.push(
        `🔴 ${file}: Duplicate function "${funcName}" (${functions[funcName]} occurrences)`,
      );
      totalIssues++;
    }
  });
      ...
      importName
        ...
        .forEach((name) => {
          ...
          if (!usedImports.has(trimmedName) && trimmedName.length > 2) {
            issues.push(
              `🟡 ${file}: Potentially unused import "${trimmedName}"`,
            );
            totalIssues++;
          }
        });
  ...
  if (content.includes("console.log")) {
    issues.push(
      `🟡 ${file}: console.log statements found (should use proper logging)`,
    );
    totalIssues++;
  }
